...
import { CHALLENGE_1_DISCLAIMER, challenge1Decision } from './challenge1Decision';
  ...
  return (
    <div className="grid gap-4">
      {Array.from({ length: 4 }).map((_, index) => (
        <div key={index} className="app-card rounded-lg p-5">
...
function scoreTone(score: number, mode: RecipientRiskTableProps['mode']) {
  if (mode === 'ghost-capacity') {
  ...
  if (score >= 81) return 'signal-badge-high';
  if (score >= 51) return 'signal-badge-medium';
  return 'signal-badge-low';
}

function signalTone(signalType: string) {
  if (
    signalType === 'disappeared_for_profit' ||
    signalType === 'for_profit_no_bn' ||
    signalType === 'pass_through' ||
    signalType === 'post_inactive_funding' ||
    signalType === 'registry_dissolution_signal'
  ) {
  ...
  if (
    signalType === 'zombie' ||
    signalType === 'no_bn' ||
    signalType === 'multi_department_for_profit' ||
    signalType === 'registry_inactive_signal' ||
    signalType === 'funding_disappearance_review'
  ) {
    ...
    <div className="space-y-3">
      ...
      {mode === 'zombies' && (
        <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-accent-soft)] px-4 py-3 text-sm leading-6 text-[var(--color-muted)]">
          <span className="font-semibold text-[var(--color-ink)]">Human-in-the-loop pilot:</span>{' '}
      ...
      <div className="grid gap-4">
        {rows.map((row) => {
          const decision = mode === 'zombies' ? challenge1Decision(row) : null;

          return (
            <article key={row.recipientKey} className="app-card rounded-lg p-5">
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="section-title">{mode === 'zombies' ? 'Zombie review case' : 'Ghost capacity screening'}</p>
                    <span className={`rounded-full px-2.5 py-1 text-xs font-semibold uppercase tracking-wide ${scoreTone(row.challengeScore, mode)}`}>
                    ...
                    {mode === 'zombies' && row.confidenceLevel && (
                  ...
                  <div className="flex flex-wrap gap-3 text-sm text-[var(--color-muted)]">
                    ...
                    <span>
                      Last seen <span className="font-medium text-[var(--color-ink)]">{row.lastYear ?? 'n/a'}</span>
                    </span>
                    {mode === 'zombies' && (
                      <span>
                  ...
                  {row.whyFlagged.length > 0 && (
                    <ul className="grid gap-2 md:grid-cols-2">
                      {row.whyFlagged.slice(0, 4).map((reason) => (
                        <li key={reason} className="rounded-lg bg-white/70 px-3 py-2 text-sm text-[var(--color-ink)]">
                          {reason}
                        </li>
                      ))}
                    </ul>
                  ...
                  {mode === 'zombies' && row.confidenceNote && (
                    <p className="rounded-lg bg-white/70 px-3 py-2 text-sm text-[var(--color-muted)]">
                      {row.confidenceNote}
                    </p>
                  )}
                  ...
                  {decision && (
                    <div className="rounded-lg border border-[var(--color-border)] bg-white/80 p-3">
                      ...
                      <p className="mt-2 text-xs text-[var(--color-muted)]">
                        Suggested reviewer: <span className="font-medium text-[var(--color-ink)]">{decision.reviewerRole}</span>
                      </p>
                    </div>
                  )}
                ...
                <div className="flex flex-col items-end gap-2">
                  <Link
                    ...
                    className="btn rounded-md border border-transparent bg-[var(--color-accent)] px-4 py-2.5 text-sm font-medium text-white hover:opacity-90"
                  >
                    Open detail
                  </Link>
                  <p className="text-xs text-[var(--color-muted)]">
