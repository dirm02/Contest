    ...
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end">
      {isOpen && (
        <div className="mb-4 flex h-[500px] w-[380px] flex-col overflow-hidden rounded-2xl border border-[var(--color-border)] bg-white shadow-2xl transition-all animate-in slide-in-from-bottom-4">
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-4 space-y-4 text-[var(--color-ink)]">
            {messages.length === 0 && (
          ...
          <form onSubmit={handleSend} className="border-t border-[var(--color-border)] p-4">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 rounded-xl border border-[var(--color-border)] bg-stone-50 px-4 py-2 text-sm outline-none focus:border-[var(--color-accent)] text-[var(--color-ink)]"
              />
              <button
                type="submit"
