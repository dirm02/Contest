...
// ── JSONL reading ────────────────────────────────────────────────
// Most files are conventional JSONL, but some exported rows contain literal
// newlines inside string fields. We rebuild each logical JSON object by
// tracking braces outside quoted strings instead of assuming 1 physical line
// always equals 1 row.
async function* streamJsonObjects(filePath) {
  ...
  for await (const line of rl) {
    ...
    for (const ch of chunk) {
      if (!started) {
        ...
        // Surface unexpected junk as a malformed record instead of silently
        // swallowing bytes that would shift subsequent parsing.
        buffer += ch;
        continue;
      }
      ...
      if (ch === '}') {
        depth--;
        if (depth === 0) {
          const jsonText = buffer.trim();
          try {
            yield { value: JSON.parse(jsonText), startLine: recordStartLine };
          } catch (e) {
            yield { error: e, startLine: recordStartLine };
          }
          reset();
        }
      }
  ...
  if (buffer.trim()) {
    yield {
      error: new Error('Unexpected end of file while reading JSON object'),
...
async function importTableData(client, schema, tableInfo, batchSize) {
  ...
  let statementRowLimit = batchSize;
  let imported = 0;
  let rowErrors = 0;
  ...
  async function flushBatch() {
    ...
    const rows = batch;
    batch = [];
    for (let i = 0; i < rows.length; i += statementRowLimit) {
      await insertRows(rows.slice(i, i + statementRowLimit));
    }
  ...
  for await (const record of streamJsonObjects(jsonlPath)) {
    if (record.error) {
      console.error(`\n  Skipping malformed JSON object in ${tableInfo.file} starting at line ${record.startLine}: ${record.error.message}`);
      continue;
    }
    ...
    if (!columns) {
      ...
      statementRowLimit = Math.max(1, Math.min(batchSize, Math.floor(MAX_BIND_PARAMS / columns.length)));
    }
    ...
    if (batch.length >= statementRowLimit) await flushBatch();
  }
  await flushBatch();
