...
This checkout already includes the exported JSONL bundle under [`dataset/`](../dataset), and `.local-db/data` is wired to that folder as a symlink. You do **not** need to download a second copy of the data.

To bring up a matching local Postgres in Docker from the repo root:
...
Once the container is healthy, install the importer dependencies and load the schemas plus data:
...
That flow creates the `cra`, `fed`, `ab`, and `general` schemas inside the local `hackathon` database exposed on `localhost:5432`.
...
In this repo, `.local-db/data` already points at the checked-in `dataset/` directory. If you move the importer elsewhere, preserve the same `data/{cra,fed,ab,general}/*.jsonl` layout.
